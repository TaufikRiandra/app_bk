...
<div style="display:grid;grid-template-columns:280px 1fr;gap:2rem;align-items:start">
	...
	<div>
		...
		<!-- Pilih Kelas -->
		<div style="position:sticky;top:22rem;background:var(--bg-secondary);padding:1.5rem;border-radius:8px;border:1px solid var(--border)">
					...
					<?php 
						...
						foreach($kelas_7 as $k):
							...
							$btn_style = $is_selected 
									? 'background:#E8E4FF;color:var(--brand)' 
									: 'background:var(--bg-light);color:var(--text-dark);border:1px solid var(--border)';
						?>
							<button type="button" onclick="loadKelasAbsen('<?= $k ?>')" style="<?= $btn_style ?>;padding:0.75rem;text-align:center;border-radius:6px;font-weight:600;cursor:pointer;border:none;font-size:0.9rem" title="Kelas <?= $k ?>">
							<?= $k ?>
						</button>
					<?php endforeach; ?>
				</div>
	...
	<div>
		<?php if(!$tanggal_terpilih || !$bulan_terpilih || !$kelas_terpilih): ?>
			<div style="background:var(--surface);border:2px dashed var(--border);border-radius:12px;padding:3rem;text-align:center">
				<div style="font-size:2.5rem;margin-bottom:1rem"><i class="fas fa-calendar"></i></div>
			...
			<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden">
				...
				<?php if(!empty($siswa_list)): ?>
					...
					<div style="background:var(--bg-light);padding:1rem;border-top:1px solid var(--border);text-align:right">
						<button onclick="simpanAbsen()" style="padding:0.75rem 2rem;background:linear-gradient(135deg, var(--brand) 0%, #5b21b6 100%);color:white;border:none;border-radius:6px;cursor:pointer;font-weight:600;box-shadow:0 4px 12px rgba(91, 78, 255, 0.3)">
