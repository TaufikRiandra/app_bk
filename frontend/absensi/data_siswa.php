...
<div style="display:grid;grid-template-columns:280px 1fr;gap:2rem;align-items:start">
	...
	<div style="sticky:top;top:2rem;background:var(--bg-light);padding:1.5rem;border-radius:8px;border:1px solid var(--border)">
		...
		<?php if($kelas_terpilih): ?>
			<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:2rem">
				<div style="margin-bottom:1.5rem">
					...
					<p style="color:var(--text-light);margin:0">Kelas <?= htmlspecialchars($kelas_terpilih) ?> <?= $has_exist_data ? '✅ (Data sudah ada)' : '📝 (Data baru)' ?></p>
				</div>
				...
				<form id="formNamaSiswa" style="display:grid;grid-template-columns:1fr;gap:1rem;max-height:600px;overflow-y:auto;padding-right:0.5rem">
					...
					<div style="margin-top:2rem;display:flex;gap:1rem">
						<button onclick="simpanSiswa()" style="padding:0.75rem 2rem;background:var(--brand);color:white;border:none;border-radius:6px;cursor:pointer;font-weight:600;transition:all 0.3s">
							<?= $has_exist_data ? '🔄 Update Data' : '💾 Simpan Data' ?>
						</button>
						<button onclick="kosongkanForm()" style="padding:0.75rem 2rem;background:var(--bg-light);color:var(--text-dark);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-weight:600;transition:all 0.3s">
							🔄 Bersihkan
						</button>
					</div>
				</div>
